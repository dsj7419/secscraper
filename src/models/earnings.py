...
class EarningsReport(AuditableModel):
    ...
    report_date: datetime = Field(
        ...,
        description="Date of earnings report"
    )
    ...
    revenue_actual: Optional[Decimal] = Field(
        default=None,
        description="Actual revenue in millions"
    )
    
    # Report metadata
    market_session: MarketSession = Field(
        default=MarketSession.UNSPECIFIED,
        description="Market session for report"
    )
    
    status: EarningsStatus = Field(
        default=EarningsStatus.TENTATIVE,
        description="Status of earnings report"
    )
    
    report_time: Optional[time] = Field(
        default=None,
        description="Specific time of report"
    )
    
    conference_call_url: Optional[str] = Field(
        default=None,
        description="URL for earnings conference call"
    )
    ...
    def calculate_surprises(self) -> None:
        ...
        if self.eps_estimate is not None and self.eps_actual is not None:
            object.__setattr__(
                self,
                "eps_surprise",
                self.eps_actual - self.eps_estimate
            )
        ...
        if self.revenue_estimate is not None and self.revenue_actual is not None:
            object.__setattr__(
                self,
                "revenue_surprise",
                self.revenue_actual - self.revenue_estimate
            )
...
class EarningsSummary(AuditableModel):
    ...
    beat_estimates: int = Field(
        default=0,
        description="Number of times earnings beat estimates"
    )
    
    missed_estimates: int = Field(
        default=0,
        description="Number of times earnings missed estimates"
    )
    
    average_surprise: Optional[Decimal] = Field(
        default=None,
        description="Average earnings surprise"
    )
    ...
    def from_reports(
        ...
        beat_estimates = sum(
            1 for r in reports
            if r.eps_surprise is not None and r.eps_surprise > 0
        )
