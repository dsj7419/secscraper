...
class Company(AuditableModel):
    ...
    name: str = Field(
        ..., description="Company legal name", min_length=1, max_length=200
    )

    exchange: Exchange = Field(
        default=Exchange.OTHER, description="Stock exchange where company is listed"
    )

    status: CompanyStatus = Field(
        default=CompanyStatus.ACTIVE, description="Current company status"
    )

    description: Optional[str] = Field(
        default=None, description="Company business description"
    )

    sector: Optional[str] = Field(
        default=None, description="Company sector classification"
    )

    industry: Optional[str] = Field(
        default=None, description="Company industry classification"
    )
