...
async def validate_data_integrity(
    cik_service: CIKService, earnings_service: EarningsService
) -> List[str]:
    """
    ...
    try:
        ...
        if invalid_ciks:
            issues.append(f"Invalid CIK format for symbols: {', '.join(invalid_ciks)}")
        ...
        if not latest_date:
            ...
            issues.append(
                f"Earnings data may be stale. Latest date: {latest_date.date()}"
            )
...
async def clean_duplicate_data(earnings_service: EarningsService) -> int:
    """
...
async def rebuild_daily_files(earnings_service: EarningsService) -> int:
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="SEC Earnings Database Maintenance")

    parser.add_argument(
        "--validate", action="store_true", help="Validate data integrity"
    )

    parser.add_argument(
        "--clean-duplicates", action="store_true", help="Remove duplicate entries"
    )

    parser.add_argument(
        "--rebuild-daily", action="store_true", help="Rebuild daily earnings files"
    )
...
async def main() -> None:
    ...
    try:
        ...
        earnings_service = EarningsService(
            nasdaq_client, cik_service, earnings_repository
        )
